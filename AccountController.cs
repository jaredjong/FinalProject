...
namespace Group12_FinalProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //Logic for change ohone number
        // GET: /Account/Changeohonenumber
        public ActionResult ChangePhoneNumber()
        {
            ...
            if (User.Identity.Name == "admin@example.com" || User.Identity.Name == "bevo@example.com")
            {
                return View("Error", new string[] { "You cannot change the phone number for the default users." });
            }
        ...
        public async Task<ActionResult> ChangePhoneNumber(ChangePhoneNumberViewModel cpnvm)
        {
            ...
            //if user forgot a field, send them back to 
            //change PhoneNumber page to try again
            if (ModelState.IsValid == false)
            {
                return View(cpnvm);
            }
            ...
            //Attempt to change the PhoneNumber using the UserManager
            var result = await _userManager.ChangePhoneNumberAsync(userLoggedIn, cpnvm.OldPhoneNumber, cpnvm.NewPhoneNumber);

            //if the attempt to change the PhoneNumber worked
            if (result.Succeeded)
            {
                //sign in the user with the new password
                await _signInManager.SignInAsync(userLoggedIn, isPersistent: false);
            ...
            else //attempt to change the password didn't work
            {
                ...
                //send the user back to the change PhoneNumber page to try again
                return View(cpnvm);
            }
        }
